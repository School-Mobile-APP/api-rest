...
import com.schoolapi.api.services.TipoViviendaService;
import com.schoolapi.api.utils.JwtUtils;

@RestController
@CrossOrigin(origins = "*")
@RequestMapping("/tipoviviendas")
public class TipoViviendaController {
	...
	public ResponseEntity<?> getAll(@RequestHeader(value = "authorization", defaultValue = "") String auth,@RequestHeader(value = "code", defaultValue = "") String code){
		try {
			...
			return ResponseEntity.status(HttpStatus.OK).body(tipoViviendaService.findAll());
		} catch (Exception e) {
	...
	public ResponseEntity<?> getOne(@PathVariable Long id,@RequestHeader(value = "authorization", defaultValue = "") String auth,@RequestHeader(value = "code", defaultValue = "") String code){
		try {
			...
			return ResponseEntity.status(HttpStatus.OK).body(tipoViviendaService.findById(id));
		} catch (Exception e) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{\"Error\":\"" + e.toString() + "\"}");
		}
	}
