...
public interface TelefonoRepository extends JpaRepository<SgTelefono, Long> {
	@Query(value = "select *" + " from sg_telefonos" + " where tel_persona=:id", nativeQuery = true)
	@Transactional(readOnly = true)
	public List<SgTelefono> getTelefonos(@PathVariable("id") Long id);

	@Modifying
	@Transactional
	@Query(value = "INSERT INTO sg_telefonos (tel_telefono, tel_persona) VALUES(?1,?2)"
	, nativeQuery = true)
	public Integer inserTelefono(String telefono, int persona);
	
	@Modifying
	@Transactional
	@Query(value = "UPDATE sg_telefonos set tel_persona=:persona, tel_telefono=:telefono"
			+ " where tel_pk=:id"
	, nativeQuery = true)
	public Integer updateTelefono(@PathVariable("persona") int persona, @PathVariable("telefono") String telefono,
			@PathVariable("id") Long id);
	@Modifying
	@Query(value = "delete from sg_telefonos t where "
